;;intended to test average age of routed message
;;from a given node A to node B

breed[vhashnodes vhashnode]
;;vhashnodes-own[peers];; only need links

breed[messages message]
messages-own[hop-dest dest age hops]

globals [done-msgs total-age average total-hops average-hops]


to setup
  reset-ticks
  clear-all-plots
  create-network popsize 
end
...
to spawn-message ;; called by god
  let myorigin one-of vhashnodes
  let mydest one-of vhashnodes
  create-messages 1 [
    move-to myorigin
    set dest mydest
    set hops 1
...
to finish-message ;; called by finishing message
  set done-msgs done-msgs + 1.0
  set total-age total-age + age
  set average total-age / done-msgs
  set total-hops total-hops + hops
  set average-hops total-hops / done-msgs
  die
end
...
to set-voronoi-peers ;; called by this turtle
  let everybody-else sort-on [distance self] other vhashnodes
...
@#$#@#$#@
<experiments>
  <experiment name="distance" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="hopstest" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>average-hops</metric>
    <enumeratedValueSet variable="popsize">
      <value value="2"/>
      <value value="4"/>
      <value value="8"/>
