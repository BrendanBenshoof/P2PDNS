...
breed[chordnodes chordnode]
chordnodes-own[id];; only need links

breed[messages message]
messages-own[hop-dest dest age hops]

globals [done-msgs total-age average total-hops average-hops maxval]
...
to create-network [n]
  ...
  ;;link up peers
  create-chordnodes n [
    set id random(maxval)
    set shape "circle"
    set color 5
    let alone 0
    ask other chordnodes-here[
      set alone 1
    ]
    while [alone = 1] [
