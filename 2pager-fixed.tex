...
In terms of metric length, the routing distance in Vhash converges to length $O(C^d)$  as the average distance between 2 nodes as the number of nodes approaches infinity. A standard DHT overlay is necessarily $O(lg(n))$ as the distance it takes for the DHT to route 1 hop is equivalent to the distance a Vhash message requires to reach the destination. 
